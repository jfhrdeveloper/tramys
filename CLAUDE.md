...
- **Convención de namespaces.** El prefijo `mis-*` (`/mi-asistencia`, `/mis-permisos`, etc.) es **exclusivo del portal del trabajador**. El admin usa nombres de dominio (`/caja`, `/sedes`, `/planilla`). Si añades una ruta admin, **no uses `mis-*`** aunque el contexto sugiera "lo mío del owner".
