...
### 2026-05-06 — Auditoría de estructura post-lanzamiento (sin cambios materiales)
- **Sin cambios materiales en código** — sesión de análisis a pedido del usuario para verificar estructura y detectar pendientes.
- **Verificado correcto:** `npx tsc --noEmit` pasa limpio; las 13 tablas del schema están todas cargadas en `DataProvider.cargar()`; `mappers.ts` cubre los 12 entidades + `ajustes` singleton; 0 `TODO(supabase)` y 0 referencias a `NEXT_PUBLIC_USE_SUPABASE` en código activo; `pagosPlanilla` y `cuadresPersonales` ya migrados a Supabase (los TODOs antiguos cerrados).
- **Pendiente bug seguridad UI — `src/middleware.ts:75-93`:** falta `/eventos` en `rutasAdmin` y `/mis-eventos` en `rutasWorker`. Ambos archivos existen y están en sus sidebars, pero un trabajador podría entrar a `/eventos` (admin) y un owner/encargado a `/mis-eventos` (worker). RLS no protege esto porque es bypass de UI, no de datos.
- **Pendiente leak de privacidad — `src/hooks/useWorkerSession.ts:6,15`:** comentario "modo demo" obsoleto y fallback que retorna "el primer trabajador activo del store" si la sesión no resuelve worker. En producción Supabase puede mostrar datos de OTRO trabajador. Debería retornar `null` (HydrationGate ya garantiza `ready=true`).
- **Pendiente menor — `src/components/layout/Topbar.tsx:25-29`:** array `NOTIFS` con datos mock hardcoded ("Marco Díaz cumple años hoy", "3 adelantos pendientes…", "Miguel T. superó su meta"). Derivar del store real o eliminar hasta implementarlo. Adicionalmente: `package.json` lista `lucide-react: ^1.8.0` (línea legacy, la moderna es 0.4xx) — verificar si es intencional. ESLint sin config local (`npm run lint` lanza wizard); recomendable `.eslintrc.json` con `{ "extends": "next/core-web-vitals" }`.

### 2026-05-03 — LANZAMIENTO: eliminado modo demo + schema Supabase reescrito + code review
- **Bloque A — Schema:** descubrimos que `docs/supabase-schema.sql` estaba completamente desincronizado del código TS (usaba `trabajador_id` en vez de `worker_id`, un solo `sueldo` en vez de las 4 tarifas, faltaban `eventos`/`accesos_temporales`/`movimientos_caja`/`ajustes`/`cuadres_personales`/`pagos_planilla`). El modo Supabase nunca habría funcionado con ese schema. Se reescribió el `.sql` desde cero (540 líneas, 13 tablas, 10 enums, ~30 policies RLS, `pg_cron` idempotente, bucket `avatars`) usando `DataProvider.tsx` + `mappers.ts` como fuente de verdad. Añadidas tablas `cuadres_personales` (sandbox del trabajador, RLS solo dueño) y `pagos_planilla`. `mappers.ts` ampliado con sus rowTo/toRow.
- **Bloque B — Eliminación de modo demo:** fusión in-place de `DataProvider.tsx` y `SessionProvider.tsx` con la implementación Supabase (preservando tipos exportados, helpers, hooks y nombres de archivo → cero cambios en los 39 imports consumidores). Borrados `DataProviderSupabase.tsx` y `SessionProviderSupabase.tsx`. Eliminado el flag `NEXT_PUBLIC_USE_SUPABASE` de toda la app (Providers, middleware, login, MiPerfilModal). Cards demo del login (`w_du`/`w_rp`/`w_at`) eliminadas — queda solo el formulario real con Supabase Auth. Lógica de dominio que faltaba en el provider Supabase portada: `updatePermiso` ahora marca asistencias del rango con `estado: "permiso"` + override de ingreso según pagado/feriado/finsemana; `addAccesoTemp` aplica `rolOtorgado` + `rol_original` en `profiles`.
- **Bloque C — Code review interconexión + sync docs:** review estático sobre los 3 roles + 20+ páginas confirmó: scope encargado consistente, helpers multi-sede usados correctamente, Realtime cubre todas las tablas, no hay estado paralelo. Tres puntos a monitorear post-lanzamiento (no bloquean): `addWorker` devuelve placeholder porque Supabase requiere invite-via-Auth, `MiPerfilModal` puede dejar email desincronizado entre `auth.users` y `profiles` si una llamada falla, `addAccesoTemp` lee `rol_original` del state local (asumiendo HydrationGate). `CLAUDE.md` reescrito sin referencias a modo demo + setup paso a paso + tablas nuevas documentadas. Glitch de encoding corregido en línea 1.
- **Verificación:** `npx tsc --noEmit` pasa limpio. `npm run build` pasa: 26 páginas estáticas, sin warnings. Reportes detallados en `.claude/LAUNCH_PLAN.md` §6 (Bloque A) y §7 (Bloques B+C).
- **Acción del usuario antes de lanzar:** ejecutar el SQL de wipe de `LAUNCH_PLAN.md §1` en su Supabase Editor, luego pegar `docs/supabase-schema.sql`, habilitar Realtime para todas las tablas, registrar el primer usuario via Auth y promoverlo a owner con `update profiles set rol='owner' where id='<uuid>';`.

### 2026-05-03 — Sesión de planificación (mi-asistencia worker)
- Sin cambios materiales en código — la sesión terminó en fase de plan, esperando confirmación del usuario antes de implementar.
- Plan acordado para próxima sesión: (a) reorganizar `mi-asistencia` para separar "Cuadre estimado / Posible ganancia" (sandbox personal) en tab "Cuadrar días" del "Resumen oficial del mes" (asistencia oficial) que se moverá al tab renombrado "Calendario oficial" + duplicar `Mis tarifas` en ambos tabs. (b) En `ModalAsistencia.tsx` colapsar el flujo de foto a un solo clic eliminando el estado intermedio `showFoto` (botón inicial pasa a `<label>` envolviendo el `<input type="file">`). (c) Fix z-index del confirm dialog: agregar prop opcional `zIndex` a `Modal.tsx` (default 300, no rompe nada) y pasar `zIndex={500}` desde `Feedback.tsx` para que el confirm "Reemplazar entrada" quede encima del `ModalAsistencia` (que usa zIndex 400).
- Causa raíz identificada del bug "el monto no sube al marcar más días": el card "Resumen del mes" del tab Cuadrar días lee de `d.asistencia` (oficial), no de `cuadresPersonales` (sandbox). La reorganización planeada lo aclara visualmente.

### 2026-05-03 — Cierre de sesión: resumen consolidado
- **Asistencia (trabajador → admin):** rediseño de `ModalAsistencia` (toggle Entrada/Salida + reloj + fecha larga + foto opcional + un solo click); hora NO editable para evitar fraudes; toast de éxito reemplaza pantalla intermedia. Implementada **verificación laxa (Opción B)**: el trabajador marca → entra como oficial al instante con `marcadoPor: "trabajador"` y `verificadoPor: null`; sub-panel en `/asistencia` admin permite verificar/desverificar/editar con scope por sede; indicadores `N/M` en `/planilla` y dot naranja/verde en perfil del trabajador.
- **Cuadre personal del trabajador:** entidad nueva `CuadrePersonal` (sandbox privado del worker, `localStorage` en demo, `TODO(supabase)` para tabla con RLS). Tab "Cuadrar días" en `mi-asistencia` ahora opera solo sobre el sandbox y muestra pill comparativo con lo oficial (✓ coincide / ≠ oficial / sin oficial). Tab "Calendario" sigue siendo la verdad oficial read-only.
- **Navegación del worker:** `SidebarWorker` reescrito con patrón `NavGroup` (Mi Panel · Mi trabajo [Asistencia, Sueldo] · Solicitudes [Adelantos, Permisos] · Eventos · Alertas). `BottomNavWorker` reorganizado: primary = Inicio/Asistencia/Sueldo/Alertas (con badge); Más = Adelantos/Permisos/Eventos/Mi perfil/Cerrar sesión.
- **Planilla y pagos:** `PagoPlanilla` extendido con `metodoPago: "efectivo"|"yape"|"transferencia"`. Modal nuevo de registro con fecha de pago editable + 3 botones de método + monto + nota; advertencia "fuera de fecha" si `fechaPago > hastaISO`. Periodos de planilla restringidos a Quincenal/Mensual (fuera Diario y Semanal). En `mi-sueldo` nueva sección "Pagos recibidos" (rango, fecha, monto, método con pill de color, estado A tiempo/+Nd) y columna "Pago" en tabla del mes; en `mi-asistencia` cintillo de pagos del mes en tab Calendario.
- **Pendiente:** persistencia real de la **foto evidencia** del modal de asistencia — el usuario decidió posponerlo por costo de Storage en Supabase si los trabajadores envían muchas fotos; cuando se decida estrategia (bucket con RLS, TTL/compresión), agregar campo `evidenciaFoto?` a `AsistenciaRec` y mostrar thumbnail + lightbox en el sub-panel de verificación. También pendiente persistir `pagosPlanilla` y `cuadresPersonales` en Supabase con RLS — ambos siguen en memoria del cliente, marcados con `TODO(supabase)`.

### 2026-05-03 — ModalAsistencia rediseñado + Verificación laxa (Opción B)
- `src/components/worker/ModalAsistencia.tsx`: reescrito. Header con avatar+sede+turno y reloj live + dot de tardanza; bloque destacado con la fecha larga ("Lunes 03 de mayo"); toggle Entrada/Salida arriba con auto-selección (sin entrada → Entrada; con entrada y sin salida → Salida) y check verde junto al tipo ya marcado; foto opcional colapsada como link discreto `+ Adjuntar foto`; footer 2 botones `Marcar tipo` + `Cerrar`. Hora **NO editable**: siempre `new Date().toTimeString().slice(0,5)`. Confirmación obligatoria si la marca pisaría un valor existente. Eliminada la pantalla intermedia "¡Entrada registrada!" — ahora un toast de éxito (`useToast` ya existente) cierra el flujo en un click.
- `AsistenciaRec` extendido (DataProvider + mappers): `marcadoPor: "trabajador" | "owner" | "encargado"`, `verificadoPor: string | null`, `verificadoAt: string`. Funciones nuevas: `verificarAsistencia(recId, actorId)` y `desverificarAsistencia(recId)`. ModalAsistencia setea `marcadoPor: "trabajador"` y `verificadoPor: null` al marcar.
- `src/app/(admin)/asistencia/page.tsx`: nuevo sub-panel "Marcaciones del trabajador" colapsable arriba de la página, con badge naranja cuando hay pendientes. Filtros: solo pendientes / todas, ventana 7/14/30 días. Cada item: avatar, sede, fecha+horas+estado, botón **Verificar** (verde) / **Editar** (abre modal existente). Si ya verificado, badge "✓ verificada" con nombre del verificador y fecha; click vuelve a desverificar. Scope: encargado solo ve marcaciones de su sede (`worker.sedeId === sedeActor.id || rec.sedeIdDia === sedeActor.id`); owner ve todas.
- Indicadores de verificación en otras vistas: (a) perfil de trabajador → tabla "Historial reciente": dot 6×6 al lado de la fecha (naranja=pendiente, verde=verificada, solo si `marcadoPor === "trabajador"`). (b) `/planilla`: nueva columna "Verif." con pill `N/M` (verificadas/marcadas por trabajador) — verde si N=M, naranja si N<M, "—" si M=0.
- Motivo: el usuario quiso un flujo de un solo click para marcar (sin pantalla intermedia), con la hora protegida (no editable) para evitar ingresos fuera de horario, y un panel donde owner/encargado audite las marcaciones del trabajador con foto sin bloquear la operación diaria. Eligió Opción B (verificación laxa) sobre A (aprobación estricta).
- Pendiente: **persistencia de la foto evidencia** está fuera de scope de esta sesión por costo de Storage en Supabase si los trabajadores envían muchas fotos. Por ahora `ModalAsistencia` solo guarda `name` del archivo en estado local y no se persiste. Cuando se decida la estrategia (Storage con TTL/compresión, S3 externo, o link a almacenamiento del usuario), agregar campo `evidenciaFoto?` al `AsistenciaRec`, mostrarla en el sub-panel de verificación con thumbnail + lightbox, y crear bucket con RLS apropiada.

### 2026-05-03 — Ajustes UX: asistencia, jaladores, perfil trabajador, feriados, adelantos→finanzas
- `src/components/ui/MultiverseCalendar.tsx`: label/title del botón verde cambiado de "Trabajé" → "Trabajó" (3ra persona). Aplica a `mi-asistencia`, perfil trabajador y perfil jalador.
- `src/app/(admin)/jaladores/page.tsx`: (a) tab Comisiones — card roja renombrada "Comisión local" mostrando `balancePer`, card azul renombrada "Comisión del jalador" mostrando `totComPer` (intercambio de valores y etiquetas). (b) Tab Asistencia — eliminada card "Promedio diario" + cálculos `promedioDiario`/`mejorDia`. (c) `ModalIngreso` rediseñado: toggle "Sí asistió / No asistió" + campo opcional "Monto vendido" (puede ser 0); botón principal "Registrar asistencia"; "No asistió" en edición elimina el registro.
- `src/app/(admin)/trabajadores/page.tsx`: tab "Perfil" — `PerfilEditor` (editor en vivo) reemplazado por ficha read-only espejo del perfil de jalador, con botón "Editar trabajador" que abre el `ModalWorker` existente. Marca "igual al día normal" en card de tarifa feriado cuando coincide.
- Feriados: en `ModalWorker` el input "Feriado" reemplazado por toggle "Feriado paga igual que día normal" (default ON); si OFF aparece input para tarifa especial. Default semilla `tarifas.feriado: 60` (= `diaNormal`) en `DataProvider`. Nuevo `ModalFeriados` accesible desde botón "Feriados" en /trabajadores: lista feriados oficiales PE del año + tabla editable de tarifa de feriado por trabajador con indicador estándar/plus y botón "Restablecer".
- `src/components/layout/Sidebar.tsx`: `/adelantos` movido del grupo "Personal" al grupo "Finanzas" en NAV_OWNER. Para encargado se creó grupo "Finanzas" con `/caja` + `/adelantos` (antes `/caja` era link suelto). `BottomNav` no se tocó (ordena por frecuencia, no por agrupación).
- Motivo: usuario pidió uniformar perfil de trabajador con el de jalador (read-only + botón editar), corregir copy en español a 3ra persona, simplificar el modelo de feriados ("igual al estándar salvo plus explícito"), separar conceptualmente "asistencia" de "monto vendido" en jaladores, y agrupar adelantos como finanza.
- Pendiente: type check pasa (`npx tsc --noEmit` sin errores). Datos existentes con `tarifas.feriado` distinto de `diaNormal` quedan respetados como override; no hay migración masiva. La "vista" de feriados quedó como modal dentro de `/trabajadores` — si se requiere una página dedicada `/feriados` con histórico de pagos, queda abierto.

### 2026-05-03 — Scoping de ajustes UX en asistencia, jaladores y trabajadores
- Sin cambios materiales en código. Sesión de scoping: el usuario pidió 6 ajustes y solicitó plan antes de implementar.
- Acordados (sin tocar todavía): (1) `MultiverseCalendar` — cambiar label/title del botón verde de "Trabajé" → "Trabajó" (líneas 4, 102, 107, 115); aplica a `mi-asistencia`, perfil de trabajador y perfil de jalador. (2) Tab Comisiones del jalador (`src/app/(admin)/jaladores/page.tsx` ~469-482): renombrar card roja "Comisión" → "Comisión local" mostrando `balancePer`, y card azul "Balance" → "Comisión del jalador" mostrando `totComPer` (intercambiar valores y etiquetas). (3) Quitar card "Promedio diario" + cálculos `promedioDiario`/`mejorDia` del tab Asistencia del jalador. (4) Perfil de trabajador (`src/app/(admin)/trabajadores/page.tsx:896`): reemplazar `<PerfilEditor>` (editor en vivo) por ficha read-only + botón "Editar trabajador" que abre el `ModalWorker` existente — patrón espejo del perfil de jalador (líneas 522-555).
- Pendientes de aclaración antes de implementar: (5) **Feriados en trabajadores** — interpretación: default de `tarifas.feriado` hereda de `diaNormal` salvo override explícito, pero falta confirmar a qué se refiere "creamos una vista" (¿página `/feriados` propia o sub-vista en `/trabajadores`?). Afecta `ingresoDia` y modelo de tarifas, no se toca hasta confirmar. (6) **Registro de jaladores** — el usuario dijo "es solo para saber si asistió o no, el monto se gana en comisiones"; planteadas 3 opciones (A: relabelar "Registrar ingreso" → "Registrar asistencia"; B: separar check de asistencia + monto opcional; C: otra). Esperando decisión.
- Motivo: el usuario quiere uniformar la UX entre perfiles de jalador y trabajador (read-only + editar), corregir copy en español (3ra persona), y simplificar el modelo de comisiones/feriados.
