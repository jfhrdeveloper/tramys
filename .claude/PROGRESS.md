...
## 🌐 8. Landing pública (`src/app/page.tsx`)

- [x] **Cara principal de Tramys** — Antes redirigía a `/login`. Ahora es una landing pública con paleta TRAMYS, fuentes Bricolage + DM Mono, mobile-first, secciones:
  - **Navbar sticky** con logo, anchors y CTA "Acceso interno" → `/login`.
  - **Hero** con headline en gradiente, badge de operación activa y CTAs (Servicios + WhatsApp).
  - **Servicios** — grid de 7 entidades: **Reniec** (DNI, certificados), **Sunarp** (registral, vehicular), **Sunat** (RUC, clave SOL), **MTC** (brevetes, récord), **Antecedentes** (penales/policiales/judiciales), **Migraciones** (pasaportes, carnés), **Relaciones Exteriores** (apostilla, legalización). Cada card tiene icono SVG en color de la entidad y borde lateral del mismo color.
  - **Proceso** — 4 pasos numerados (DM Mono).
  - **Ventajas** — Rápido / Confiable / Presencial / Seguimiento.
  - **CTA final** en gradiente brand con botones de WhatsApp y llamada.
  - **Footer** con anchors + acceso interno.
- Diseño totalmente responsivo: el menú secundario se oculta en `<720px` y el grid de servicios se adapta con `auto-fit, minmax(280px, 1fr)`.
